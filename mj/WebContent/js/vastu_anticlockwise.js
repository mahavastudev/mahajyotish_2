...
function drawCircle1() {
	//("#### code to draw circle over image ####");
...
function showCuspDetail1(forward) {
	...
	//console.log('showCuspDetail1 lineTo ['+x+', 300] ')
	context.stroke();
	context.save();
...
function showCuspDetail(forward) {
	...
	//edited By gaurav Sharma spave be redline and text on 14 June 2020, add two new lines
	//var textRadius = radius + 80;
	var textRadius = radius + 85;
	...
	for (var i = 1; i < rowCount; i++) {
		...
		for (var j = 2; j <= 8; j++) {
			//console.log(" --- ["+"#tdPlanetAspect" + j + pname+"] "+$("#tdPlanetAspect" + j + pname).html());
			if ($("#tdPlanetAspect" + j + pname).html() != "") {
				// aspectStr=aspectStr+planetArr[$("#tdPlanetAspect"+j+"0").html()]+" "+$("#tdPlanetAspect"+j+pname).html()+",";
				var tempstr = $("#tdPlanetAspect" + j + pname).html().split("(");
				
				aspectStr = aspectStr + planetArr[$("#tdPlanetAspect" + j + "0").html()]+ " " + tempstr[0] + ",";
		...
		//console.log('showCuspDetail aspectStr ['+aspectStr+']');
		...
		if (pname != "") {
			if (signArr[sign] != null) {
				...
				//edited By gaurav Sharma rotate circle anticlock wise on 14 June 2020, add two new lines
				//console.log('showCuspDetail :: angle  ['+(angle-degree)+'], degree ['+degree+'], angle+degree ['+angle+']');
				angle =  180-angle; 
...
function showHouseDetailOnPlanet(forward) {
	...
	var house = "";
	var aspectStr = "";
	var lineRadius = radius + 10;
	
	//Edited By gaurav Sharma to insrease space bw arc and roman numbers , 14 June 2020
	...
	for (var i = 0; i < rowCount - 1; i++) {
		...
		for (var j = 1; j <= 7; j++) {
			if ($("#tdHouseAspect" + j + houseNO).html() != "") {
				...
				aspectStr = aspectStr+ planetArr[$("#tdHouseAspect" + j + "0").html()] + " "+ tempStr[0] + ",";
				//console.log("showHouseDetailOnPlanet ["+aspectStr+"]");
		...
		if (signArr[sign] != null) {
			...
			//edited By gaurav Sharma rotate circle anticlock wise on 14 June 2020, add two new lines
			//console.log('angle  ['+(angle-degree)+'], degree ['+degree+'], angle+degree ['+angle+']');
			angle =  180-angle; 
			...
			context.setLineDash([ 0, 0 ]);
			context.lineWidth = 1;
			//console.log('line raduis ['+lineRadius+']');
			context.arc(centerX, centerY, lineRadius, degreesToRadians(angle),degreesToRadians(angle), false);
			//console.log(degreesToRadians(angle)+'--------');
			//console.log(centerX +","+ centerY +","+ lineRadius +","+ degreesToRadians(angle) +","+degreesToRadians(angle) +","+ false);
			...
			context.translate(textRadius, 0);
			context.rotate(degreesToRadians(90));
			//console.log(degreesToRadians(90)+'----- 90 ---');
			context.font = "18px sans-serif";
			var text = house + "(" + timeArr[0] + ":" + timeArr[1] + "/"+ (pos + 1) + ")";
			//console.log("showHouseDetailOnPlanet ["+house+"]");
			//context.fillText(text, 0, 0);
			...
			if (aspectStr != "") {
				...
				for (var k = 0; k < temp.length - 1; k++) {
					context.fillText(temp[k] + String.fromCharCode(176), 0, y);
					//console.log(temp[k] + String.fromCharCode(176), 0, y);
					y = y - 13;
				}
			}
			context.restore();
