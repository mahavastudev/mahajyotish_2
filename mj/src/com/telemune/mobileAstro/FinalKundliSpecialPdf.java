...
public class FinalKundliSpecialPdf {
	static Logger logger = Logger.getLogger(FinalKundliSpecialPdf.class);
	...
	public void Kundli(AstroBean astrobean, AstroBean transitAstrobean, String FileName, boolean dasha,
		...
		try {
			 ...
			 */
			fillHouse(cuspChartDetail, canvas, this.xCoordinate, toXYCord(this.yCoordinate), this.width, this.height,
					mycolor);
			...
			List<AstroLifeContexts> astroLifeContextList = collectAstroLifeContextTableData(astrobean, properties);
			...
			astroLifeTable.writeSelectedRows(0, -1, 50, (842 - 256), writer.getDirectContent());
	...
	PdfPTable createPDFAstroLife(List<AstroLifeContexts> astroLifeContextList, ColorElement mycolor,
		...
		try {
			...
			/*
			 * Font fontplanet = new Font(); fontplanet.setSize(this.getTableDatafont());
			...
			bhavaTable.addCell(new Phrase(new Chunk("A", bhavafont)));

			bhavaTable.addCell(new Phrase(new Chunk("B", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("C", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("D", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("E", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("F", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("G", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("H", bhavafont)));
			bhavaTable.addCell(new Phrase(new Chunk("I", bhavafont)));

			bhavaTable.addCell(new Phrase(new Chunk("Hit From", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("R", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Planets (Traits)", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Deg", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Signs", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Mindset/Approach ", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Deg", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Box (Life Context)", bhavafontHead)));
			bhavaTable.addCell(new Phrase(new Chunk("Hit From", bhavafontHead)));
			int temp = 0;
			String[] temp_ = properties.getOrDefault("astro.enviornment", "").split("#,#");
			logger.info("astro.enviornment " + properties.getOrDefault("astro.enviornment", "nodata"));

					
			
			for (AstroLifeContexts k : astroLifeContextList) {
				//logger.info("###########inside AstroLifeContexts loop##############" + k);
				...
				if (temp_.length >= (temp + 1)) {
					bhavaTable.addCell(new Phrase(new Chunk(temp_[temp], bhavafont)));
				} else {
					bhavaTable.addCell(new Phrase(new Chunk("", bhavafont)));
				}
				temp++;
		...
		try {
			...
			for (String sign : this.aspectLifeContextsign) {
				...
				for (HouseDetailBean i : list) {
					temp = temp + 1;
					logger.info(" found  list is [" + list.size() + "] houseDetailBean " + i.toString()
							+ " cusp Map key " + cusp.containsValue(i) + " signName " + signName);
					...
					if (cuspKey != null) {
						...
						if (lifeContext.getCuspDetailDegree() == null) {
							lifeContext.setCuspDetailDegree(i.getDegree());
						} else {
						...
						if (lifeContext.getCuspDetailLife() == null) {
							...
							if (this.romanNumber.length >= (Integer.parseInt(cuspKey) - 1)) {
								lifeContext.setCuspDetailLife(
										this.romanNumber[cuspBoxKey - 1] + " " + boxlife[cuspBoxKey - 1] + "");
						...
						logger.info("################ contaxt box [" + boxlife[cuspBoxKey - 1] + "] , key [" + cuspKey
								+ "], roman number [" + this.romanNumber[(Integer.parseInt(cuspKey) - 1)] + "]");
									...
									if (lifeContext.getCuspDetailHitFrom() == null) {
										lifeContext.setCuspDetailHitFrom(
												planetOcuupant.getOrDefault(planets[j], " /").split("/")[0] + " "
														+ FinalKundliSpecialPdf.planetShortNameMap
					...
					for (PlanetDetailBean tempPlanetBean : list1) {
						...
						if (lifeContext.getPlanetDetailDegree() == null) {
							...
							lifeContext.setPlanetDetailDegree(tempPlanetBean.getDegree());
							...
							if (!lifeContext.getPlanetDetailDegree().contains(tempPlanetBean.getDegree())) {
								lifeContext.setPlanetDetailDegree(
										lifeContext.getPlanetDetailDegree() + "\n" + tempPlanetBean.getDegree());
							}
									...
									if (lifeContext.getPlanetDetailHit() == null) {
										lifeContext.setPlanetDetailHit(
												planetOcuupant.getOrDefault(k1, " /").split("/")[0] + " "
														+ FinalKundliSpecialPdf.planetShortNameMap.getOrDefault(k1, k1)
														+ " " + v);
										temp_1++;
									} else {
										/*
					...
					logger.info("temp value [" + temp + "] size [" + list.size() + "]");
					//logger.info("list size life context Table [" + lifeContext.toString() + "]");

					if (temp == list.size()) {
						logger.info(" Going to add into a list" + lifeContextList.add(lifeContext));
					}

				}
			}
			//logger.info(" list size table data of life contexts is [" + lifeContextList.toString() + "]  ");
		} catch (
	...
	public void fillHouse(Kundli HouseDetail, PdfContentByte content, float x, float y, float width, float height,
		...
		logger.debug("fill House x [" + x + "] y [" + y + "] width [" + width + "] height [" + height + "] ["
				+ HouseDetail.toString() + "]");
		for (int i = 0; i < 12; i++) {
			switch (i) {
			case 0:
	...
	public PdfPTable fillPanetTable_first(String[][] tabulardata, ColorElement mycolor, byte flag, String[] addHouse,
		...
		try {
			...
			Planettable.addCell(new Phrase(new Chunk("Lordship", fontplanethead)));
			/**
			 * Commented By vishal Requirement By BD Garg Sir.
			 * 
	...
	public PdfPTable fillPersonaldetail(AstroBean astrobean, ColorElement mycolor) {
		...
		try {
			...
			subpersonalDetailTable.addCell(new Phrase(new Chunk(astrobean.getDOB().trim(), font)));
			subpersonalDetailTable.getDefaultCell().setBorderWidth(0);

			subpersonalDetailTable.addCell(new Phrase(new Chunk(astrobean.getPOB().trim(), font)));
			subpersonalDetailTable.getDefaultCell().setBorderWidth(0);
	...
	public void generateMahadashaAndAntardashaTable(Vector<MahaDashaBean> dasha, PdfContentByte canvas,
		...
		if (!StringUtils.isBlank(dasha.get(0).getYear())) {
			// logger.info("**************||||||||||||||||||Antardashaaaaaaaaaaaaaaaaaaa||||||||||||||||||"
			// + dasha.get(0).getYear());
