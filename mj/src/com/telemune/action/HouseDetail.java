...
import com.telemune.model.Range;

import java.util.*;

public class HouseDetail {
	private static Logger logger = Logger.getLogger(HouseDetail.class);
	String[] sign = { "Aries", "Taurus", "Gemini", "Cancer", "Leo", "Virgo", "Libra", "Scorpio", "Sagittarius",
	...
	public void getHouseDetails(AstroBean astrobean) {
		logger.info("Inside getHouseDetails method of HouseDetail action Class.");
		...
		try {
			...
			for (int k = 1; k <= 12; k++) {
				...
				if (HouseDetailHashTable.containsKey(Integer.toString(k))) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug(
								" house cusp >>>>>  " + HouseDetailHashTable.get(Integer.toString(k)).getPlanetName());
					}
					...
					while (it.hasNext()) {
						...
						if (tmp[0].equalsIgnoreCase(planetDetail.getPlanetName())) {
							starbean.setSS(starbean.getSS() + " " + planetDetail.getSignName() + " "
									+ planetDetail.getDegree());
						}

						tmp = subLordbean.getSS().split(" ");
						if (tmp[0].equalsIgnoreCase(planetDetail.getPlanetName())) {
							subLordbean.setSS(subLordbean.getSS() + " " + planetDetail.getSignName() + " "
									+ planetDetail.getDegree());
						}

						tmp = starbean.getNL().split(" ");
						if (tmp[0].equalsIgnoreCase(planetDetail.getPlanetName())) {
							starbean.setNL(starbean.getNL() + " " + planetDetail.getSignName() + " "
									+ planetDetail.getDegree());
						}

						tmp = subLordbean.getNL().split(" ");
						if (tmp[0].equalsIgnoreCase(planetDetail.getPlanetName())) {
							subLordbean.setNL(subLordbean.getNL() + " " + planetDetail.getSignName() + " "
									+ planetDetail.getDegree());
						}
					...
					degree = (Integer.parseInt(temp[0]) * 60 * 60) + (Integer.parseInt(temp[1]) * 60)
					...
					for (int i = 0; i < angles.length; i++) {
						...
						for (int p = 0; p < planetList.size(); p++) {
							...
							while (it.hasNext()) {
								...
								// planetList name
								// "+planetList.get(p).getName());
								if (!planetDetail.getPlanetName()
										.equalsIgnoreCase(HouseDetailHashTable.get(Integer.toString(k)).getPlanetName())
										&& !planetDetail.getPlanetName().equalsIgnoreCase("Lagna")) {
									if (planetDetail.getSignName().equalsIgnoreCase(planetList.get(p).getName())) {
										...
										pdegree = (60 * 60 * Integer.parseInt(tmp[0])) + (60 * Integer.parseInt(tmp[1]))
												+ Integer.parseInt(tmp[2]);
										String aspect = "";
										if (logger.isDebugEnabled()) {
											logger.debug("RC  :: " + planetDetail.getRC() + "  "
													+ planetList.get(p).isForward() + " "
													+ planetDetail.getPlanetName());
										}
										if ((planetDetail.getRC().indexOf("R") >= 0 && planetList.get(p).isForward())
												|| (planetDetail.getRC().indexOf("R") < 0
														&& !(planetList.get(p).isForward()))
												|| (planetList.get(p).isForward() && (planetDetail.getPlanetName()
														.equalsIgnoreCase("Rahu")
														|| planetDetail.getPlanetName().equalsIgnoreCase("Ketu")))) {
											if (pdegree <= (planetList.get(p)).getEnd()
													&& pdegree >= (planetList.get(p)).getStart()) {
												String house = " ";
												if (astrobean.getCuspPlanetHashTable()
														.get(planetDetail.getPlanetName()) != null) {
													house = (astrobean.getCuspPlanetHashTable()
															.get(planetDetail.getPlanetName())).getHouseNumber()
															+ " house";
												}

												aspect = element + " "
														+ HouseDetailHashTable.get(Integer.toString(k)).getDegree()
														+ "_" + planetDetail.getPlanetName() + "_" + house + "_"
														+ planetDetail.getSignName() + "_" + minSec + "_" + angles[i];
						...
						for (int i = 0; i < angles.length; i++) {
							...
							for (int p = 0; p < planetList.size(); p++) {
								...
								while (it.hasNext()) {
									...
									if (!planetDetail.getPlanetName().equalsIgnoreCase(
											HouseDetailHashTable.get(Integer.toString(k)).getPlanetName())
											&& !planetDetail.getPlanetName().equalsIgnoreCase("Lagna")) {
										if (planetDetail.getSignName().equalsIgnoreCase(planetList.get(p).getName())) {
											minSec = planetDetail.getDegree();
											String[] tmp = minSec.split(":");
											pdegree = (Integer.parseInt(tmp[0]) * 60 * 60)
													+ (Integer.parseInt(tmp[1]) * 60) + Integer.parseInt(tmp[2]);
											String aspect = "";
											if (logger.isDebugEnabled()) {
												logger.debug("RC  :: " + planetDetail.getRC() + "  "
														+ planetList.get(p).isForward() + " "
											...
											if ((planetDetail.getRC().indexOf("R") >= 0
													&& planetList.get(p).isForward())
													|| (planetDetail.getRC().indexOf("R") < 0
															&& !(planetList.get(p).isForward()))
													|| (planetList.get(p).isForward()
															&& (planetDetail.getPlanetName().equalsIgnoreCase("Rahu")
																	|| planetDetail.getPlanetName()
																			.equalsIgnoreCase("Ketu")))) {
												if (pdegree <= (planetList.get(p)).getEnd()
														&& pdegree >= (planetList.get(p)).getStart()) {
													String house = " ";
													if (astrobean.getCuspPlanetHashTable()
													...
													if (!occList.get(o).getPlanetName()
															.equalsIgnoreCase(planetDetail.getPlanetName())) {
														aspect = occList.get(o).getSignName() + " "
																+ occList.get(o).getDegree() + "_"
																+ planetDetail.getPlanetName() + "_" + house + "_"
																+ planetDetail.getSignName() + "_" + minSec + "_"
																+ angles[i];
														if (logger.isDebugEnabled()) {
															logger.debug("aspect   " + aspect);
														}
			...
			logger.error("Inside catch block. Exception at getHouseDetails method of HouseDetail action Class. Exception  : "+e.toString());
			e.printStackTrace();
		}
