...
public class FinalKundli {
	...
	public void Kundli(AstroBean astrobean, String FileName, boolean dasha, boolean houseDetail, boolean circle,
			String circleImage, String listImage, boolean aspectChartWidoutHouse, boolean nakshatraPadam) {

		logger.info("Inside Kundli method of FinalKundli action Class.");
		...
		try {
			...
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase("Natal Strength", fontHead1), 290,
					842 - 40, 0);
			...
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase("Birth Kundli", fontHead), 160,
					842 - 98, 0);
			drawRectangle(canvas, this.xXCordinate, toXYCord(this.yCoordinate) - this.height, this.width, this.height);// Generate
																														// Cusp
																														// Kundli
			drawLine(canvas, this.xXCordinate, toXYCord(this.yCoordinate), width, height);
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase("Cusp Kundli", fontHead), 410, 842 - 98,
					0);

			fillHouse(birthChartDetail, canvas, this.xCoordinate, toXYCord(this.yCoordinate), this.width, this.height,
					mycolor);
			fillHouse(cuspChartDetail, canvas, this.xXCordinate, toXYCord(this.yCoordinate), this.width, this.height,
					mycolor);

			canvas.stroke();
			canvas.stroke();

			// **************************************To Fill Personal detail on
			// first page***************************************//
			...
			// *****************************************To Fill Planet Detail On
			// first page***************************************//
			String[][] tabulardata = new String[9][7];
			String[][] tabulardata_1 = new String[9][7];// Pankaj
			...
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase("MahaJyotish Script", fontKrishna), 290,
					842 - 226, 0);
			...
			krishanTable.writeSelectedRows(0, -1, 50, (842 - 501), writer.getDirectContent());

			// *************************************To fill Bhava Detail on
			// first page*********************************************//
			...
			bhavaTable.writeSelectedRows(0, -1, 50, (842 - 651), writer.getDirectContent());
			...
			if (houseDetail || aspectChartWidoutHouse) {
				houseDetailing(canvas, astrobean, mycolor, writer, houseDetail, aspectChartWidoutHouse);
			}
	...
	public void houseDetailing(PdfContentByte canvas, AstroBean astrobean, ColorElement mycolor, PdfWriter writer,
			boolean houseDetail, boolean aspectChartWidoutHouse) {

		logger.info("Inside houseDetailing method of FinalKundli action Class.");
		float width1 = 220;
		float height1 = 110; // old110
		float xCoordinate1 = 80;
		float yCoordinate1 = 160; // old 160
		logger.info("aspectChartWidoutHouse>> " + aspectChartWidoutHouse + "  houseDetail>> " + houseDetail);
		try {
				...
				for (int j = 0; j < planets.length; j++) {
					...
					if (!planets[j].equals("Ketu") && !planets[j].equals("Rahu")) {
						...
						for (int count = 0; count < planets.length; count++) {
							// if(!planets[j].equals("Ketu") &&
							// !planets[j].equals("Rahu")){
	...
	public void DashaStrength(PdfContentByte canvas, AstroBean astrobean, ColorElement mycolor) {
		...
		logger.info("Inside DashaStrength method of FinalKundli action Class.");
		float width1 = 220;
		float height1 = 110; // old110
		float xCoordinate1 = 80;
		float yCoordinate1 = 160; // old 160
		...
		try {
			...
			if (obj.size() != 0) {
				for (int j = 0; j < obj.size(); j++) {
					...
					drawRectangle(canvas, xCoordinate1, toXYCord(yCoordinate1) - height1, width1, height1);// Draw
																											// Cusp
																											// Kundli
																											// of
																											// second
	...
	public void mahaDashaAndSooksham(PdfContentByte canvas, PdfContentByte content, AstroBean astrobean,
		...
		logger.info("Inside mahaDashaAndSooksham method of FinalKundli action Class.");
		try {
			...
			ColumnText.showTextAligned(canvas, Element.ALIGN_LEFT,
					new Phrase(astrobean.getName().replaceAll("%20", " ").replaceAll("\"", ""), font1), 35, 842 - 55,
					0);
	...
	public static void drawRectangle(PdfContentByte content, float x, float y, float width, float height) {
		logger.info("Inside drawRectangle method of FinalKundli action Class.");
		try{
		content.saveState();
	...
	public static void drawLine(PdfContentByte content, float x, float y, float width, float height) {
		...
		logger.info("Inside drawLine method of FinalKundli action Class.");
		try{
	...
	public void fillHouse(Kundli HouseDetail, PdfContentByte content, float x, float y, float width, float height,
			ColorElement mycolor) {
		logger.info("Inside fillHouse method of FinalKundli action Class.");
	...
	public PdfPTable fillPanetTable_first(String[][] tabulardata, ColorElement mycolor, byte flag, String[] addHouse,
		...
		logger.info("Inside fillPanetTable_first method of FinalKundli action Class.");
			...
			for (int outer = 0; outer < tabulardata.length; outer++) {
				for (int inner = 0; inner < tabulardata[outer].length + 1; inner++) {
					...
					if (flag == 1 && inner == 3) {
						...
						logger.info("The Value Of Inner is [" + inner + "] And Outer is [" + outer
	...
	public PdfPTable fillPanetTable(String[][] tabulardata, ColorElement mycolor, byte flag, String[] addHouse)
		...
		logger.info("Inside fillPanetTable method of FinalKundli action Class.");
		Font fontplanet = new Font();
		fontplanet.setSize(this.getTableDatafont());
		fontplanet.setColor(mycolor.fillColor(this.getTableDataColor()));
	...
	public PdfPTable fillAspecttable(AstroBean astrobean, ColorElement mycolor) {
		logger.info("Inside fillAspecttable method of FinalKundli action Class.");
	...
	public PdfPTable fillPersonaldetail(AstroBean astrobean, ColorElement mycolor) {
		logger.info("Inside fillPersonaldetail method of FinalKundli action Class.");
		if (logger.isDebugEnabled())
		...
		try {
			...
			subpersonalDetailTable.addCell(new Phrase(new Chunk(" ")));// To
																		// have
																		// a
																		// space
			...
			if (logger.isDebugEnabled())
				logger.debug("Error -------- [ " + astrobean.getDOB().substring(astrobean.getDOB().lastIndexOf(" "),
						astrobean.getDOB().length()));
			subpersonalDetailTable.addCell(new Phrase(new Chunk("TOB:", fonthead)));
			subpersonalDetailTable.getDefaultCell().setBorderWidth(0);
			subpersonalDetailTable.addCell(new Phrase(new Chunk(
					astrobean.getDOB().substring(astrobean.getDOB().lastIndexOf(" "), astrobean.getDOB().length()),
					font)));
			subpersonalDetailTable.getDefaultCell().setBorderWidth(0);
			...
			if (!StringUtils.isBlank(Dob)) {
				...
				if (logger.isDebugEnabled())
					logger.debug("******************||||||The first index of Date of Birth|||||||||||||******* [0] [ "
							+ DobArr[0] + "] [1]  " + DobArr[1]);

				subpersonalDetailTable.addCell(new Phrase(new Chunk("DOB:", fonthead)));
				subpersonalDetailTable.getDefaultCell().setBorderWidth(0);
				subpersonalDetailTable
						.addCell(new Phrase(new Chunk(DobArr[0] + " " + DobArr[1] + " " + DobArr[2], font)));
				subpersonalDetailTable.getDefaultCell().setBorderWidth(0);
			}
			subpersonalDetailTable.addCell(new Phrase(new Chunk(" ")));// To
																		// have
	...
	public PdfPTable krishanKundliTable(AstroBean astrobean, ColorElement mycolor, boolean nakshatraPadam) {
		logger.info("Inside krishanKundliTable method of FinalKundli action Class.");
		PdfPTable krishanTable = null;
		float[] wid;
		try {

			if (nakshatraPadam) {
				krishanTable = new PdfPTable(9);
			...
			for (int i = 0; i < planets.length; i++) {
				planetDetail = planetHousesHashTable.get(planets[i]);
	...
	public PdfPTable fillBhavaTable(AstroBean astrobean, ColorElement mycolor, boolean nakshatraPadam) {
		logger.info("Inside fillBhavaTable method of FinalKundli action Class.");
		...
		try {
			...
			for (int k = 1; k <= 12; k++) {
				if (HouseDetailHashTable.containsKey(Integer.toString(k))) {
					...
					if (nakshatraPadam) {
						bhavaTable.addCell(new Phrase(
								new Chunk(HouseDetailHashTable.get(Integer.toString(k)).getNakshatra(), bhavafont)));
						bhavaTable.addCell(new Phrase(
								new Chunk(HouseDetailHashTable.get(Integer.toString(k)).getPadam(), bhavafont)));
					}
					bhavaTable.addCell(
							new Phrase(new Chunk(HouseDetailHashTable.get(Integer.toString(k)).getRL(), bhavafont)));
					bhavaTable.addCell(
							new Phrase(new Chunk(HouseDetailHashTable.get(Integer.toString(k)).getNL(), bhavafont)));
					bhavaTable.addCell(
							new Phrase(new Chunk(HouseDetailHashTable.get(Integer.toString(k)).getSL(), bhavafont)));
	...
	public PdfPTable fillAspecttable(DashaStrength dashaBean, ColorElement mycolor) {
		...
		try {
			...
			if (RahuAspect != null) {
				String RahuAspectstr = "";
				logger.info("RahuAspect[0] : " + RahuAspect[0]);
	...
	public PdfPTable fillPersonalDetail(AstroBean dashaAstroBean, ColorElement mycolor, DashaStrength dashaBean) {
		...
		try {
			...
			if (!StringUtils.isBlank(Dob)) {
				...
				DobArr = Dob.split(" ");
				DobArr[1] = DobArr[1].substring(0, 3);
				// logger.info("******************||||||The first index of Date
				// of Birth|||||||||||||*******"+DobArr[0]+DobArr[1]);
	...
	public void generateMahadashaAndAntardashaTable(Vector<MahaDashaBean> dasha, PdfContentByte canvas,
		...
		logger.info("Inside generateMahadashaAndAntardashaTable method of FinalKundli action Class.");
		Font font = new Font();
		font.setSize(this.getTableDatafont());
		...
		if (!StringUtils.isBlank(dasha.get(0).getYear())) {
			...
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(monthArr[0], font3), tableX + 20,
					tableY + 15, 0);
			ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(monthArr[1], font), tableX + 35,
					tableY + 4, 0);
	...
	public static Map sortByComparator(Map unsortMap) {
		...
		logger.info("Inside sortByComparator method of FinalKundli action Class.");
	...
	public void drawLine1(PdfContentByte content, float x, float y, float width, float height) {
		logger.info("Inside drawLine1 method of FinalKundli action Class.");
		content.saveState();
		content.moveTo(x, y);
	...
	public void fillAntarDashaTable(PdfContentByte canvas, AstroBean astrobean, ColorElement mycolor) {
		...
		logger.info("Inside fillAntarDashaTable method of FinalKundli action Class.");
		try {
	...
	public void generateSookshamDashaTable(PdfContentByte canvas, Vector<MahaDashaBean> sookshamDashaVector,
		...
		logger.info("Inside generateSookshamDashaTable method of FinalKundli action Class.");
		try {
			MahaDashaBean mahadashabean = null;
	...
	public void fillSookshmaDashaTable(PdfContentByte canvas, AstroBean astrobean, ColorElement mycolor)
		...
		logger.info("Inside fillSookshmaDashaTable method of FinalKundli action Class.");
